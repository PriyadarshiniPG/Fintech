...
const codeceptNpmScript = 'codeceptjs';

function main() {
  // Decide if tests are going to be executed with CodeceptJS
  ...
  try {
    ...
    // needed to avoid feeding the arguments to npm itself, and instead pass them through to the CodeceptJS script
    execSync(`npm --prefix "${globals.coreDir}" run ${codeceptNpmScript} -- ${_.join(args, ' ')}`, {
      stdio: 'inherit',
    });
  } catch {
    if (run) {
