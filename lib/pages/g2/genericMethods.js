...
class GenericMethods extends Helper {
  async makerChecker() {
    ...
    let noOfEle = await I.grabNumberOfVisibleElements(
      CommonUtils.identifyLocator('GenericLocators.MakerChecker'),
    );
    if (noOfEle !== 0) {
      let makerCheckerElementInDiv = await I.grabTextFrom(
        CommonUtils.identifyLocator('GenericLocators.MakerChecker'),
      );
  ...
  async changeAccountstatusTable(status, account) {
    await I.scrollIntoView(CommonUtils.identifyLocator('GenericLocators.ScrollExport'), 'false');
    //await G2Handlers.waitForPageLoad();
    await I.wait(3);
  ...
  async initiateTransaction(tObj) {
    ...
    if (number == 0) {
      ...
      await I.scrollIntoView(CommonUtils.identifyLocator('InitiateTransPage.TranService'), 'true');
      //Trans service
      ...
      //Amount
      await I.scrollIntoView(CommonUtils.identifyLocator('InitiateTransPage.Amount(M)'), 'false');
      ...
      if (popup ==!0 || popup1==!0) {
        ...
        await G2Handlers.waitForPageLoad();
        let receiptNum = await I.grabTextFrom(
          "//label[text()='Receipt No.']//following-sibling::div",
        );
        TestData.setField('transactionReceiptNum', receiptNum);
        let status = await I.grabTextFrom("//label[text()='Status']//following-sibling::div");
        TestData.setField('strStatus', status);
      }
      let PopError = await G2Handlers.grabNumberOfVisibleElements("//div[@role='dialog']//div[@class='el-message-box__title']//span[text()='Error']", i++);
      if (PopError !== 0) {
      ...
      await G2Handlers.clickOnElement(' View Detail >> ', 'buttonToView');
      await G2Handlers.waitForPageLoad();
      I.waitForElement(await CommonUtils.identifyLocator('InitiateTransPage.ReceiptNo'));
      let receiptNum = await I.grabTextFrom(
        "//p[text()=' Receipt No.  ']//following-sibling::p[@class='descriptions-item__content']",
      );

      TestData.setField('transactionReceiptNum', receiptNum);
      let status = await I.grabTextFrom(
  ...
  async clickOnAllButton(btnName) {
    ...
    if (btnName.toLowerCase() == 'pass') {
      ...
      noOfEle = await I.grabNumberOfVisibleElements(locator);
    } else {
  ...
  async editorgKYCInfo(tObj) {
    ...
    await I.scrollIntoView(CommonUtils.identifyLocator('GenericLocators.ScrollContactNo'), 'true');
  ...
  async getTotalRecords(tableID) {
    ...
    let count = await I.grabTextFrom("label[id='" + tableID + "_page_totalRecord']");
    if (parseInt(count) >= 0) {
  ...
  async downloadFile(fileType) {
    ...
    let downloadedFileNames = await I.grabFileNames();
  ...
  async getValueFromTableBasedonRowAndColumn(tableID, rowNum, columnNum, outputVar) {
    tableID = CommonUtils.identifyData(tableID);
    let tableVal = await I.grabTextFrom(
  ...
  async verifyValuesAreSortedInAscendingOrder(firstVal, secondVal) {
    ...
    if (secondVal >= firstVal) {
      I.report('Values are sorted in ascending order');
    } else {
  ...
  async verifyValuesAreSortedInDescendingOrder(firstVal, secondVal) {
    ...
    secondVal = TestData.getField(secondVal);
    if (secondVal <= firstVal) {
      I.report('Values are sorted in descending order');
    } else {
      throw new Error('Values are not sorted');
    }
  }
  ...
  async verifyTheTextInTheKycPopUp(textString) {
    let value = await I.grabTextFrom('//body');
    value = value.trim;
  ...
  async checkIfEditIdentityStatusButtonAvailable() {
    if (
      I.grabNumberOfVisibleElements(CommonUtils.identifyLocator('orgpage.Identity Status Edit')) >=
      1
    ) {
  ...
  async navigateToTillPageForCreation(pageName, tObj) {
    ...
    I.scrollIntoView(CommonUtils.identifyLocator('GenericLocators.UnselectAllButton'), 'false');
    I.wait('5');
  ...
  async createCustomerdetails(tObj) {
    ...
    await I.scrollIntoView(CommonUtils.identifyLocator('GenericLocators.registeredType'), 'false');
    await G2Handlers.enterValue('MSISDN', msisdn, 'textboxUsingLabel');
    //await G2Handlers.enterValue('MSISDN', msisdn, 'textboxUsingLabel');
    await G2Handlers.enterValue('First Name', firstname, 'textboxUsingLabel');
    await G2Handlers.enterValue('Last Name', lastname, 'textboxUsingLabel');
