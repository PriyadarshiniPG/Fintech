...
class SearchPage {
  ...
  async searchCustomerAndResetPIN(custMSISDN, key) {
    ...
    if (count === 0) {
      I.report(actMsisdn.MSISDN + ' Cusustomer not created');
    } else {
  ...
  async generateInputAndCallAPI(projCode, startPIN, activateCustMSISDN, soapAction) {
    ...
    var algoName = CommonUtils.identifyData(projCode + '.api.encryptionAlgo');
    var callerKeyValue = CommonUtils.identifyData(projCode + '.api.callerKey');
    let encryptInputObj = {
    ...
    var enPassword = CommonUtils.executeMethodFromJAR(
      encryptInputObj,
      CommonUtils.identifyData(projCode + '.api.callerPassword'),
    );
    var inputDataObj = {
    ...
    await SoapApi.callApi(soapAction, apiKeys, inputDataObj, projCode);
  }
}
