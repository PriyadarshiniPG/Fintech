...
class AdminPage {
  ...
  async fillRequiredFieldsForIdentityInfoSp(userName, Language, Channel) {
    ...
    await G2Handlers.clickOnElement(Channel, 'buttonToView');
  }
  ...
  async fillRequiredFieldsForIdentityInfoOrg(
    ...
    I.scrollIntoView(await CommonUtils.identifyLocator('GenericLocators.ScrollLanguage'), 'false');
  ...
  async createSpOperator(table) {
    ...
    await G2Handlers.clickOnElement(CommonUtils.identifyData(Gender), 'buttonToView');
    if (Gender !== 'undefined') {
      await G2Handlers.clickOnElement(CommonUtils.identifyData(Gender), 'buttonRadio1');
    }
    await I.scrollIntoView(CommonUtils.identifyLocator('GenericLocators.AddID'), 'false');
  ...
  async createOrgOperator(table) {
    let tObj = table.parse().hashes()[0];
    let { ShortCode, Language, UserName, MSISDN, OperatorId, Email } = {
      ...tObj,
    };
    let firstName = 'Auto',
      lastName = 'mation';
    ...
    await G2Handlers.verifyText('Personal Details');
    await this.fillRequiredFieldForPersonDetails(firstName, lastName, Email);
    await G2Handlers.clickOnElement(Gender, 'buttonToView');
  ...
  async changeNotificationChannel(operatorRef) {
    ...
    if (Object.keys(items).length > 0) {
      ...
      await HomePage.clickOnLogout();
      I.report('The Notification Channel is changed for the Short Code ' + shortCode);
    }
  }
  ...
  async recycleMSISDN(msisdn) {
    ...
    if (number == 0) {
      ...
      I.report('This MSISDN is already recycled or not assigned to any customer or till.');
    }
  }
