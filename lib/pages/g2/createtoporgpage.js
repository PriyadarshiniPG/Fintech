...
class CreateTopOrgPage {
  ...
  async createTopOrg(table) {
    ...
    await I.scrollIntoView(CommonUtils.identifyLocator('GenericLocators.ScrollProduct'), 'true');
    ...
    if (number == 0) {
      // Review Comment - assert.fail will fail the test cases so replacing with I.report. If validation mode is not mendatory then goto next steps instead of failing test
      //assert.fail('The Validation Mode is not an mandatory field');
      I.report('The Validation Mode is not an mandatory field');
    } else {
      await G2Handlers.selectOption(
        'Validation Mode',
        'External Validation',
  ...
  async createTill(table) {
    ...
    await I.scrollIntoView(CommonUtils.identifyLocator('orgpage.Product'), 'false');
