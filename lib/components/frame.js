...
class IFrame extends Helper {
  ...
  async switchToPopUpWindow() {
    ...
    await I.switchToWindow(windows[windows.length - 1]);
    await I.switchTo("//iframe[contains(@id,'popwin')]");
  }
  ...
  async switchToErrorPopUpWindow() {
    ...
    const windows = await I.grabAllWindowHandles();
    await I.switchToWindow(windows[windows.length - 1]);
    await I.switchTo("(//iframe[contains(@id,'popwin')])[2]");
  }
}
