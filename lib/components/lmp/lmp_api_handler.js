const { CommonUtils, I, LoggerFactory, TestData } = inject();
...
class LmpApi {
  ...
  async setFlow(endpoint) {
    this.endpointData = TestData.getData(`lmp.api.${endpoint}`);
   ...
   */
  resetFlow() {
    this.commonData = this.commonData || TestData.getData('lmp.api.common');
    this.environmentData = this.environmentData || TestData.getData('lmp.api.environment');
  ...
  async validateResponse() {
    await I.report('Validating response');
