const { I, LoggerFactory, CommonUtils } = inject();
...
class SoapApi {
  ...
  renderApiKeys(reqName, apiKeys, projCode) {
    ...
    for (const apiKey of apiKeys) {
      if (typeof activateCustReq != 'undefined' && activateCustReq.includes(apiKey)) {
        activateCustReq = activateCustReq.replace(
          ...
          CommonUtils.identifyData(projCode + '.api.' + apiKey),
        );
      }
    }
    ...
    this.getResultsUrl = CommonUtils.identifyData(projCode + '.api.getResultsURL');
    this.getResultMethod = CommonUtils.identifyData(projCode + '.api.getResultMethod');
  ...
  async validateResponse(verifyValue) {
    ...
    if (actualResStatus == expectedResStatus) {
      ...
      await I.fail('Received unexpected Status code ' + actualResStatus);
    }
    ...
    if (typeof valExpData != 'undefined' && valExpData !== null) {
      ...
      if (valActualData.includes(valExpData)) {
        ...
        await I.fail('Response description not includes ' + valExpData);
      }
    } else {
      await I.report('Expected Data not provided');
    }
    await I.report('Response validated');
  }
}
