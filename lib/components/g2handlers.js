...
const { Assert, I, CommonUtils, TestData, LoggerFactory } = inject();
...
class G2Handlers extends Handlers {
  ...
  buttonRadio1 = locate(
    ".//label[contains(@class,'el-radio')]//span[normalize-space(text())='REPLACE_LOCATOR']",
  );
  ...
  async verifyText(locator) {
    await this.waitForPageLoad();
    await I.waitForText(CommonUtils.identifyData(locator), global.explicitWait);
    await Assert.verifyText(CommonUtils.identifyData(locator));
    await I.report('Verified text ' + CommonUtils.identifyData(locator));
  }

  async waitForPageLoad() {
    let pageLoadlocator = CommonUtils.identifyLocator('GenericLocators.pageLoadlocator');
    await I.waitForInvisible(pageLoadlocator, 30);
  }

  async verifyTextNotPresent(locator) {
    await this.waitForPageLoad();
    await Assert.verifyTextNotPresent(locator);
    await I.report('Verified text ' + CommonUtils.identifyLocator(locator) + ' is not displayed');
  }
  ...
  async checkLessThan(value1, compare, value2) {
    ...
    I.report('Checked that ' + value1 + ' is ' + compare + '' + value2);
  }
  ...
  async checkValueisGreaterThan(strTempString1, strTempString2, strAmount) {
    ...
    if (intTempString1 >= intTempString2 + intAmount) {
      I.report('The Value is Credited');
    } else if (intTempString1 > intTempString2 - intAmount) {
      I.report('The Value is Credited with some charges deducted');
    } else {
      I.fail('The amount is not Credited');
    }
  }
  ...
  async checkValueisLessThan(strTempString1, strTempString2, strAmount) {
    ...
    if (intTempString1 <= intTempString2 - intAmount) {
      I.report('The Value is Debited');
    } else {
      I.fail('The amount is not Debited');
    }
  }
  ...
  async getValueFromTable(tableId, uniqueValues, columnNumber, outputVar) {
    ...
    let value = await I.grabTextFrom(path);
    value = value.trim();
  ...
  async clickValueOnTableData(tableId, uniqueValues, operation) {
    ...
    if (menuItem == 'Till') {
      ...
      path = locate("(.//*[./@class = '" + tableId + "']//tr//*[text()=' " + operation + " '])");
  ...
  async clickLinkOnTableData(tableId, uniqueValues, operation, columnName) {
    ...
    let columnIndex = await I.grabAttributeFrom(columnLocator, 'colmarker');
    path = path.find(
      '//td[' +
        ++columnIndex +
        "]//a[contains(text(),'" +
  ...
  async getValueFromLabel(label, storeOutput) {
    ...
    if (typeof TestData.getLocator(label) !== 'undefined') {
      ...
      value = await I.grabTextFrom(CommonUtils.identifyLocator(label));
    } else {
      ...
      await I.waitForElement(locator, global.explicitWait);
      value = await I.grabTextFrom(locator);
    }
