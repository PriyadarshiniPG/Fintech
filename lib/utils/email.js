  ...
  async getMessage(subject, since = Date.now() - 24 * 3600 * 1000) {
    if (!this.credentialsConfigured) {
      let errorMsg = utils.email.enabled
        ? 'Email credentials are not configured, set GAF_UTILS_EMAIL_PASSWORD and GAF_UTILS_EMAIL_USER env variables'
        : 'Email utility is not enabled, set GAF_UTILS_EMAIL to true';
